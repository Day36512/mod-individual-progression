        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_FLIGHT_ICEBOLT:
            {
                ...
                if (!targets.empty() && iceboltCount)
                {
                    ...
                    me->CastSpell(*itr, SPELL_ICEBOLT_TRIGGER, true);  // stun the player immediately
                    me->CastSpell(*itr, SPELL_ICEBOLT_CAST, false);
                    blockList.push_back((*itr)->GetGUID());
                    currentTarget = (*itr)->GetGUID();
                    --iceboltCount;
                    events.ScheduleEvent(EVENT_FLIGHT_ICEBOLT, (me->GetExactDist(*itr) / 13.0f) * IN_MILLISECONDS + 1200);  // Add 2000 ms delay
                }
                else
                {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            for (auto& target : targets)
            {
                if (CAST_AI(boss_sapphiron_40::boss_sapphiron_40AI, caster->ToCreature()->AI())->IsValidExplosionTarget(target))
                {
                    if (target->IsPlayer())
                    {
                        // Assuming target->ToPlayer() would give you a Player pointer, and that
                        // Player class has a method IsNPCBot()
